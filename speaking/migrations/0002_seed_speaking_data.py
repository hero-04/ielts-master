...
def seed_speaking(apps, schema_editor):
    SpeakingTest = apps.get_model('speaking', 'SpeakingTest')
    SpeakingQuestion = apps.get_model('speaking', 'SpeakingQuestion')
...
def unseed_speaking(apps, schema_editor):
    SpeakingTest = apps.get_model('speaking', 'SpeakingTest')
    SpeakingTest.objects.filter(
        cambridge_book__in=[10, 11],
        test_number__in=[1, 2],
    ).delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_speaking, reverse_code=unseed_speaking),
    ]
