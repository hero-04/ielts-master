...
export default function ListeningBookPage() {
  ...
  const [testsByNumber, setTestsByNumber] = useState<Record<number, ListeningTest>>({});
  ...
  useEffect(() => {
    const fetchTests = async () => {
      try {
        ...
        const map: Record<number, ListeningTest> = {};
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <Link
          href="/listening"
          className="inline-flex items-center gap-2 text-gray-500 hover:text-purple-600 transition-colors mb-4 text-sm font-medium"
        >
          <ArrowLeft className="w-4 h-4" />
          Back to books
        </Link>
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Cambridge {bookNumber}</h2>
        <p className="text-gray-600 text-lg">Select a test to begin your IELTS listening practice.</p>
      </div>
                ...
                <div key={testNum} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex flex-col">
                  ...
                  <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
                    <Link
                      ...
                      className="w-full flex items-center justify-center gap-2 py-2.5 bg-purple-600 text-white font-semibold rounded-xl hover:bg-purple-700 transition-colors"
                    >
